    ...
    var body: some View {
        VStack {
            ...
            .overlay(alignment: .bottom) {
                Button {
                    if goal != "" {
                        let habit = Habit(name: name, goal: Decimal(string: goal) ?? 0.0, progress: 0, type: type)
                        viewModel.add(habit: habit)
                        dismiss()
                    }
                } label: {
                    Text("Save")
...
#Preview {
    SHCreateHabitView(viewModel: HabitViewModel())
}
